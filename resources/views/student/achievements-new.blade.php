    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="hidden md:flex items-center">
            <div class="bg-white bg-opacity-20 backdrop-filter backdrop-blur-sm rounded-lg px-6 py-3 text-white">
                <span class="font-bold text-2xl">{{ $totalScore }}</span>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <div class="stats-card bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg p-4 flex items-center transform transition-all duration-300 hover:scale-105 hover:shadow-lg">
        ...
        <div>
            ...
            <div class="text-white text-2xl font-bold">{{ $totalScore }}</div>
        </div>
    </div>
    ...
    <div class="stats-card bg-gradient-to-r from-green-600 to-green-700 rounded-lg p-4 flex items-center transform transition-all duration-300 hover:scale-105 hover:shadow-lg">
        ...
        <div>
            ...
            <div class="text-white text-2xl font-bold">{{ $quizzesTaken }}</div>
        </div>
    </div>
    ...
    <div class="stats-card bg-gradient-to-r from-amber-600 to-amber-700 rounded-lg p-4 flex items-center transform transition-all duration-300 hover:scale-105 hover:shadow-lg">
        ...
        <div>
            ...
            <div class="text-white text-2xl font-bold">{{ count(array_filter($achievements, function($a) { return $a['unlocked']; })) }}</div>
        </div>
    </div>
    ...
    <div class="section-content bg-gray-900 rounded-b-lg p-6">
        ...
        <div class="relative mb-6">
            ...
            <div class="w-full bg-gray-700 rounded-full h-4 mb-8">
                <div class="bg-primary-600 h-4 rounded-full transition-all duration-1000 ease-out" style="width: {{ $progressPercentage }}%"></div>
            </div>
            ...
            <div class="absolute right-0 top-10 md:top-0 transform md:translate-y-0">
                <div class="relative w-40 h-40">
                    ...
                    <svg class="w-full h-full" viewBox="0 0 36 36">
                        ...
                        <path class="stroke-current text-primary-500" stroke-width="3" fill="none" stroke-dasharray="{{ $progressPercentage }}, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"></path>
                    </svg>
                    <div class="absolute inset-0 flex items-center justify-center">
                        <div class="text-center">
                            <div class="text-white text-5xl font-bold">{{ round($progressPercentage) }}%</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-16">
            ...
            @foreach($tiers as $key => $tier)
                <div class="text-center">
                    <div class="w-16 h-16 rounded-full bg-{{ $tier['color'] }}-900 bg-opacity-20 flex items-center justify-center mx-auto mb-3">
                        <i class="fas fa-{{ $tier['icon'] }} text-{{ $tier['color'] }}-400 text-2xl"></i>
                    </div>
                    ...
                    <div class="w-full bg-gray-700 rounded-full h-2">
                        <div class="bg-{{ $tier['color'] }}-500 h-2 rounded-full transition-all duration-1000 ease-out" style="width: {{ $tier['total'] > 0 ? ($tier['count'] / $tier['total']) * 100 : 0 }}%"></div>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="section-content bg-gray-900 rounded-b-lg p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($achievements as $ach)
                ...
                <div class="bg-gray-800 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 {{ $ach['unlocked'] ? '' : 'opacity-60' }}">
                    <div class="flex items-center p-4">
                        <div class="w-10 h-10 rounded-full {{ $tierColor['bg'] }} flex items-center justify-center mr-3">
                            <i class="fas fa-{{ $tierColor['icon'] }} text-white"></i>
                        </div>
                        <div>
                            <h3 class="text-white font-bold">{{ $ach['title'] }}</h3>
                            <div class="text-xs text-gray-400 uppercase font-semibold">{{ $ach['tier'] }} TIER</div>
                        </div>
                        <div class="ml-auto">
                            @if($ach['unlocked'])
...
<div class="section-card mb-8">
    ...
    <div class="section-content bg-gray-900 rounded-b-lg p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-gray-800 rounded-lg p-5 text-center {{ $totalScore >= 100 ? '' : 'opacity-80' }}">
                ...
                @else
                    <div class="relative bg-gray-700 text-gray-500 font-medium py-2 px-4 rounded-lg w-full cursor-not-allowed">
                        ...
                        <div class="w-full bg-gray-800 rounded-full h-1 mt-1">
                            <div class="bg-purple-600 h-1 rounded-full" style="width: {{ min(100, ($totalScore / 100) * 100) }}%"></div>
                        </div>
                    </div>
                @endif
            </div>

            <div class="bg-gray-800 rounded-lg p-5 text-center {{ $totalScore >= 300 ? '' : 'opacity-80' }}">
                ...
                @else
                    <div class="relative bg-gray-700 text-gray-500 font-medium py-2 px-4 rounded-lg w-full cursor-not-allowed">
                        <div class="flex justify-between items-center">
                            <span>Locked</span>
                            <span>{{ $totalScore }}/300</span>
                        </div>
                        <div class="w-full bg-gray-800 rounded-full h-1 mt-1">
                            <div class="bg-blue-600 h-1 rounded-full" style="width: {{ min(100, ($totalScore / 300) * 100) }}%"></div>
                        </div>
                    </div>
                @endif
            </div>

            <div class="bg-gray-800 rounded-lg p-5 text-center {{ $totalScore >= 500 ? '' : 'opacity-80' }}">
                ...
                @else
                    <div class="relative bg-gray-700 text-gray-500 font-medium py-2 px-4 rounded-lg w-full cursor-not-allowed">
                        ...
                        <div class="w-full bg-gray-800 rounded-full h-1 mt-1">
                            <div class="bg-yellow-600 h-1 rounded-full" style="width: {{ min(100, ($totalScore / 500) * 100) }}%"></div>
                        </div>
                    </div>
                @endif
            </div>
