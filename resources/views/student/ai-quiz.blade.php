...
@section('title', 'AI Quiz - ' . $course->title)
...
document.getElementById('generateQuizForm').addEventListener('submit', function(e) {
    ...
    fetch(`/student/ai-quiz/{{ $course->id }}/generate`, {
        method: 'POST',
        headers: {
...
document.addEventListener('submit', function(e) {
    if (e.target.id === 'quizForm') {
        ...
        fetch(`/student/ai-quiz/{{ $course->id }}/submit`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    quizContainer.innerHTML = `
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 text-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <a href="{{ route('student.ai.quiz', $course->id) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg transition-colors inline-block text-center">
                    <i class="fas fa-brain mr-2"></i>New AI Quiz
                </a>
                <a href="{{ route('student.showCourse', $course->id) }}" class="bg-gray-600 hover:bg-gray-700 text-white font-medium py-3 px-4 rounded-lg transition-colors inline-block text-center">
                    <i class="fas fa-arrow-left mr-2"></i>Back to Course
                </a>
            </div>
