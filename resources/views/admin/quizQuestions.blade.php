...
@section('title', $quiz->name . ' - Questions')
...
<div class="bg-gradient-to-r from-blue-900 via-primary-800 to-secondary-900 rounded-xl shadow-2xl p-6 mb-8 border border-blue-700/30 relative overflow-hidden">
    ...
    <div class="relative flex flex-col md:flex-row justify-between items-center">
        ...
        <div class="mt-4 md:mt-0">
            <a href="{{ route('admin.createQuestion', $quiz->id) }}" class="group bg-white/10 backdrop-blur-sm border border-white/20 text-white hover:bg-white/20 font-semibold py-3 px-5 rounded-lg shadow-lg transition-all duration-300 inline-flex items-center hover:shadow-purple-500/20 hover:shadow-xl">
                    ...
                    <tr class="question-row border-b border-gray-800/80 hover:bg-gray-800/50 transition-all duration-200">
                        <td class="px-4 py-4">
                            <div class="flex items-start">
                                ...
                                <div class="question-text text-white">{{ $question->question }}</div>
                            </div>
                        </td>
                        <td class="px-4 py-4">
                            <div class="answers-list text-gray-300">{{ $question->answers }}</div>
                        </td>
                        <td class="px-4 py-4">
                            @php
                        ...
                        <td class="px-4 py-4">
                            <div class="flex gap-2">
                                <a href="{{ route('admin.editQuestion', $question->id) }}"
                                ...
                                <form action="{{ route('admin.deleteQuestion', $question->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
